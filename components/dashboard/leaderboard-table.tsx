...
export function LeaderboardTable({ tokens }: { tokens: ArenaToken[] }) {
  return (
    <section className="flex flex-col gap-3 rounded-2xl border border-[var(--line)] bg-[var(--panel-bg)] p-[14px] shadow-[0_10px_24px_rgba(2,7,16,.35)] transition hover:-translate-y-[1px] hover:border-[#4b66a3] hover:shadow-[0_16px_32px_rgba(2,7,16,.45)] lg:col-span-3">
              ...
              <tr key={token.symbol} className="transition hover:bg-[#16243f]">
                ...
                <td className="border-b border-[var(--line)] px-2 py-2.5 text-[0.84rem]">
                  <svg viewBox="0 0 120 34" className="h-[34px] w-[120px]" aria-hidden="true">
                    <path d={linePath(token.priceChange, 120, 34, 4)} fill="none" stroke="#ffd166" strokeWidth="2.5" />
                  </svg>
                </td>
              </tr>
            ))}
